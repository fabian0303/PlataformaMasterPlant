...
export const fetchUsers = (userId = '') => {
  return async (dispatch, getState) => {
    ...
    if (userId) {
      ...
      if (!user) {
        const errorMessage = 'Usuario no disponible';
...
export const deleteUser = (id) => {
  return async (dispatch, getState) => {
    ...
    toastr.success('', 'El usuario fue eliminado.');
...
export const createUser = ({
  ...
  return async (dispatch, getState) => {
    ...
    const createUserDbTask = createDocument('users', uidAux, userData);
    ...
    return dispatch(USERS_CREATE_USER_SUCCESS({ user: uidAux }));
  };
};
